...
import type { NFATransition } from '../types';

export function getEpsilonClosure(states: string[], transitions: NFATransition[]): string[] {
  const closure = new Set<string>(states);
  const stack = [...states];
  while (stack.length > 0) {
    const s = stack.pop()!;
    for (const t of transitions) {
      if (t.from === s && t.symbol === EPSILON && !closure.has(t.to)) {
        closure.add(t.to);
        stack.push(t.to);
      }
    }
...
export function subsetKey(states: string[]): string {
  return states.length === 0 ? '__empty__' : states.slice().sort().join(',');
}

export function subsetDisplay(states: string[]): string {
  return states.length === 0 ? '∅' : `{${states.slice().sort().join(',')}}`;
}

export function subsetNodeId(states: string[]): string {
  return states.length === 0 ? 'dfa_empty' : `dfa_${states.slice().sort().join('_')}`;
}